...
namespace fcitx {
    ...
    void Sequencer::push_action(const MicroStep& step) {
        ...
        if (step.type == MicroStepType::PassthroughNonVietnameseKey) {
            if (!queue_.empty() && queue_.back().type == MicroStepType::PassthroughNonVietnameseKey) {
                if (queue_.back().keycode == step.keycode) {
                    LOTUS_INFO("🛡️ [SEQUENCER DEDUP] Coalesced duplicate non-Vietnamese key event in queue");
                    return;
                }
            }
        }

        LOTUS_INFO("📥 [SEQUENCER PUSH] Action queued [Serial #" + std::to_string(step.serial) + "]: type=" + std::to_string(static_cast<int>(step.type)) + " text=" + step.text +
        ...
        if (step.type == MicroStepType::EmitBackspace) {
            ...
            LOTUS_INFO("🛡️ [TOKEN COUNT] Added expected uinput backspace swallow token [Serial #" + std::to_string(step.serial) + "]: count=" + std::to_string(step.count) +
    ...
    uint64_t Sequencer::calculate_adaptive_delay_ms(uint64_t measured_ack_ms) {
        ...
        // Adaptive formula: min_delay_ms <= adaptive <= max_ack_timeout_ms (15ms)
        uint64_t adaptive = std::clamp(measured_ack_ms + 1, config_.min_delay_ms, config_.max_ack_timeout_ms);
        LOTUS_INFO("📊 [ADAPTIVE DELAY] Measured App ACK: " + std::to_string(measured_ack_ms) + "ms -> Dynamic Barrier: " + std::to_string(adaptive) + "ms");
        return adaptive;
    }
    ...
    void Sequencer::set_waiting_ack() {
        ...
        LOTUS_INFO("🛡️ [SEQUENCER BARRIER] Set WaitingForAck (Niri Frame ACK) for Serial #" + std::to_string(active_serial_.load()));
    }
    ...
    void Sequencer::receive_ack(uint32_t serial) {
        if (barrier_ == BarrierState::WaitingForAck) {
            if (serial >= active_serial_.load(std::memory_order_acquire)) {
                ...
                LOTUS_INFO("✅ [SERIAL ACK RELEASED] Compositor responded for Serial #" + std::to_string(serial) + " elapsed=" + std::to_string(elapsed) + "ms");
                barrier_ = BarrierState::Ready;
            } else {
                LOTUS_INFO("🛡️ [SERIAL STALE DISCARD] Discarded out-of-order ACK Serial #" + std::to_string(serial) + " (active: #" + std::to_string(active_serial_.load()) + ")");
            }
        }
    }
    ...
    bool Sequencer::poll_next_step(MicroStep& out_step) {
        ...
        if (barrier_ == BarrierState::WaitingMicroDelay) {
            if (now >= barrier_target_time_) {
                LOTUS_INFO("⏱️ [MICRO-DELAY DONE] Adaptive delay completed, releasing barrier");
                barrier_ = BarrierState::Ready;
            } else {
                return false;
            }
            ...
            if (elapsed >= static_cast<int64_t>(config_.max_ack_timeout_ms)) {
                LOTUS_INFO("⏱️ [SAFETY TIMEOUT] " + std::to_string(config_.max_ack_timeout_ms) + "ms limit reached without ACK signal. Unblocking Serial #" +
        ...
        LOTUS_INFO("⚡ [SEQUENCER EXECUTE] Dispatching MicroStep [Serial #" + std::to_string(out_step.serial) + "]: type=" + std::to_string(static_cast<int>(out_step.type)) +
                   " text=" + out_step.text);
    ...
    bool Sequencer::should_swallow_backspace(uint32_t serial) {
        if (serial > 0 && serial < active_serial_.load(std::memory_order_acquire)) {
            LOTUS_INFO("🛡️ [SERIAL STALE DISCARD] Ignored backspace from old Serial #" + std::to_string(serial));
            return false;
        }
        if (expected_swallow_backspaces_.load(std::memory_order_acquire) > 0) {
            ...
            LOTUS_INFO("🛡️ [SWALLOW BACKSPACE] Consumed uinput backspace token [Serial #" + std::to_string(active_serial_.load()) +
