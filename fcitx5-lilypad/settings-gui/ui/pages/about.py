...
class AboutPage(QWidget):
    ...
    def _on_export_logs(self):
        ...
        save_dialog_result = QFileDialog.getSaveFileName(
            ...
            os.path.expanduser("~/fcitx5-lilypad-debug.log"),
        ...
        try:
            with open(export_filename, 'w') as log_output_file:
                log_output_file.write("=== Fcitx5 Lilypad Debug Log Export ===\n")
                log_output_file.write(f"Version: {__version__}\n")
                log_output_file.write(f"User: {getpass.getuser()}\n")
                log_output_file.write("--------------------------------------\n\n")

                system_log_path = os.path.join(tempfile.gettempdir(), "fcitx5-lilypad-server.log")
                ...
                log_output_file.write("--- Systemd Journal (fcitx5-lilypad-server) ---\n")
                try:
                    current_sys_user = getpass.getuser()
                    process_capture = subprocess.run(
                        ['journalctl', f'-u', f'fcitx5-lilypad-server@{current_sys_user}.service', '--no-pager', '-n', '200'],
