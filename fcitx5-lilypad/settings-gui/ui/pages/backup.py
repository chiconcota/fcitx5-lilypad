...
"""
Backup and Restore page for Lilypad settings.
...
class BackupPage(QWidget):
    def __init__(self, dbus_handler: LilypadDBusHandler, parent=None):
    ...
    def _get_local_dict_path(self) -> str:
        ...
        return os.path.join(xdg_data_home, "fcitx5/lilypad/vietnamese.cm.dict")
    ...
    def do_export(self):
        ...
        default_filename = (
            f"lilypad-backup-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
        )
        ...
        try:
            ...
            if selected["macros"]:
                macros = self.dbus.get_sub_config_list("lilypad-macro", "Macro")
    ...
    def on_restore_selected(self):
        ...
        try:
            ...
            if "macros" in selected_keys and "macros" in backup:
                self.dbus.set_sub_config_list("lilypad-macro", "Macro", backup["macros"])
