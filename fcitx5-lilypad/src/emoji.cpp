...
EmojiLoader::EmojiLoader(fcitx::AddonManager* addonManager) {
    ...
    historyPath_ = (fcitx::StandardPaths::global().userDirectory(fcitx::StandardPathsType::Config) / "fcitx5" / "conf" / "lilypad-emoji-history.conf").string();
#else
    historyPath_ = fcitx::StandardPath::global().userDirectory(fcitx::StandardPath::Type::Config) + "/fcitx5/conf/lilypad-emoji-history.conf";
#endif
    loadFromFcitx5("en");
    loadHistory();
}
