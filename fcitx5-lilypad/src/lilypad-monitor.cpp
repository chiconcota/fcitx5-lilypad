...
void        mousePressResetThread() {
    ...
    LILYPAD_INFO("Mouse press reset thread started.");
    ...
    while (!stop_flag_monitor.load(std::memory_order_acquire)) {
        ...
        LILYPAD_INFO("Mouse socket connected.");
        ...
        while (!stop_flag_monitor.load(std::memory_order_acquire)) {
            ...
            if (ret > 0 && ((pfd.revents & POLLIN) != 0)) {
                ...
                if (n <= 0) {
                    LILYPAD_ERROR("Mouse socket recv error: " + std::string(strerror(errno)));
                    break;
                }
                ...
                if (getsockopt(sock, SOL_SOCKET, SO_PEERCRED, &cred, &len) == 0) {
                    ...
                    if (fd >= 0) {
                        if (read(fd, exe_path, sizeof(exe_path) - 1) < 0) {
                            LILYPAD_ERROR("Failed to read cmdline: " + std::string(strerror(errno)));
                        }
                        close(fd);
                    }
                }

                if (strcmp(exe_path, "/usr/bin/fcitx5-lilypad-server") == 0) {
                    LILYPAD_DEBUG("Mouse click detected from server. Resetting engine.");
                    needEngineReset.store(true, std::memory_order_release);
                    g_mouse_clicked.store(true, std::memory_order_release);
                } else {
                    LILYPAD_WARN("Unauthorized connection attempt from: " + std::string(exe_path));
                }
            } else if (ret < 0 && errno != EINTR) {
                LILYPAD_ERROR("Mouse socket poll error: " + std::string(strerror(errno)));
                break;
            }
        }
        close(sock);
