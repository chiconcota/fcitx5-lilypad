...
#include "lilypad-server.h"
#include "lilypad-logger.h"
...
bool UinputDevice::initialize() {
    ...
    strncpy(usetup.name, "Lilypad-Uinput-Server", UINPUT_MAX_NAME_SIZE - 1);
...
void boost_process_priority() {
    if (setpriority(PRIO_PROCESS, 0, -10) != 0) { //NOLINT
        LilypadLogger::instance().error("Failed to boost process priority");
    }
}
...
void pin_to_pcore() {
    ...
    if (sched_setaffinity(0, sizeof(cpuset), &cpuset) != 0) {
        LilypadLogger::instance().error("Failed to pin process to core");
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (expected_uid == (uid_t)-1) {
        LilypadLogger::instance().error("Failed to find UID for target user: " + target_user);
        return 1;
    }
    ...
    if (!uinput.initialize()) {
        LilypadLogger::instance().error("Failed to initialize uinput device");
        return 1;
    }
    ...
    if (bind(server_fd.get(), (struct sockaddr*)&addr_kb, kb_len) != 0) {
        LilypadLogger::instance().error("Failed to bind socket");
        return 1;
    }
    ...
    if (!li_ctx.is_valid()) {
        LilypadLogger::instance().error("Failed to create libinput/udev context");
        return 1;
    }
    ...
    while (g_running.load(std::memory_order_acquire)) {
                ...
                if (getsockopt(client_fd, SOL_SOCKET, SO_PEERCRED, &cred, &len) == 0) {
                    if (cred.uid == expected_uid) {
                        ...
                        if (strcmp(exe_path, "/usr/bin/fcitx5") == 0) {
                            ...
                            LilypadLogger::instance().warn("Unauthorized executable connection attempt to keyboard socket from: " + std::string(exe_path));
                        }
                    } else {
                        LilypadLogger::instance().warn("Unauthorized UID connection attempt to keyboard socket from UID: " + std::to_string(cred.uid));
                    }
                } else {
                    LilypadLogger::instance().warn("Failed to get peer credentials for keyboard socket");
                }

                if (authorized) {
                    LilypadLogger::instance().info("Fcitx5 connected to keyboard socket (PID: " + std::to_string(cred.pid) + ")");
        ...
        if (fds[KB_CLIENT_INDEX].fd >= 0 && (fds[KB_CLIENT_INDEX].revents & (POLLIN | POLLHUP | POLLERR)) != 0) {
            ...
            if (n <= 0) {
                LilypadLogger::instance().warn("Keyboard client disconnected or connection error");
        ...
        if ((fds[2].revents & POLLIN) != 0) {
            ...
            if (new_fd >= 0) {
                LilypadLogger::instance().info("New mouse flag client connected");
        ...
        if ((fds[1].revents & POLLIN) != 0) {
            ...
            while ((event = libinput_get_event(li_ctx.get_li())) != nullptr) {
                ...
                if (type == LIBINPUT_EVENT_POINTER_BUTTON) {
                    ...
                    if (libinput_event_pointer_get_button_state(p) == LIBINPUT_BUTTON_STATE_PRESSED) {
                        if (addon_fd.is_valid()) {
                            if (send(addon_fd.get(), "C", 1, MSG_NOSIGNAL | MSG_DONTWAIT) <= 0) {
                                LilypadLogger::instance().warn("Failed to send to mouse flag client, closing connection");
                                addon_fd.reset(-1);
                            }
                        }
                    }
                    ...
                    LilypadLogger::instance().info("Device added: " + std::string(name));
    ...
    LilypadLogger::instance().info("Terminating server...");
    return 0;
}
